...
int main() {
    const auto image = cv::imread("../images/moss.jpg", 0);
    // std::cout << image.rows << image.cols << std::endl;
    cv::Mat fimage;
    image.convertTo(fimage, CV_32F, 1.0/255);
    cv::Mat dimage;
    cv::dft( fimage, dimage );
    for (size_t i = 0; i < 100; ++i) {
        for (size_t j = 0; j < 100; ++j) {
            dimage.at<float>(dimage.rows - i - 1, dimage.cols - j - 1) = 0.0;
        }
    }
    cv::dct( dimage, dimage, cv::DCT_INVERSE );
    ...
    //         cropped.at<cv::Vec3b>(i, j) = img.at<cv::Vec3b>(i + 100, j + 100);
    //     }
    // 
    // }
